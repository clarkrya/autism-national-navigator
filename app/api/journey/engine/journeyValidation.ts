 ...
 *
 * This file provides the application-level validation layer
 * between AI output and the family-facing website.
 *
 * The validator checks that:
 *
 * 1. The journey follows the family's stated priority.
 * 2. Unrelated domains are removed where appropriate.
...
function shouldRemoveInsuranceRecommendation(
   ...
   * not become the primary recommendation unless insurance
   * was explicitly identified as the issue.
   */
...
function filterBlockedDomainItems(
  ...
  return items.filter(
    (item) => {
      ...
      /*
       * Generic blocked-domain protection.
       */
      ...
      /*
       * Keep an item when it is clearly about the selected
       * priority even though another domain is mentioned.
       */
...
function validateUnsureJourney(
  ...
  /*
   * Remove explicit school actions from an unsure journey.
   */
 ...
 * IMPORTANT:
 *
 * This function now uses the shared FamilyProfile type from:
 *
 *   types/familyProfile.ts
 *
 * This prevents the route and validator from using separate
 * incompatible FamilyProfile definitions.
 * ============================================================
 */
...
export function validateJourney(
   ...
   * ----------------------------------------------------------
   *
   * School remains context when therapy is the family's
   * selected priority.
   */
   ...
   * ----------------------------------------------------------
   *
   * If therapy is already being received, do not blindly
   * tell the family to find therapy again.
   */
    ...
    validated.tasks =
      (
        ...
        (task) => {
          ...
          /*
           * Remove generic find-a-therapist actions.
           */
  ...
  /*
   * Keep arrays predictable for the front end.
   */
